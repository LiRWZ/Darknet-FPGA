...
static std::vector<detection> dets_copy;
static network net;
static image in_s ;
static image det_s;
...
//***tracker
static std::vector<tracker> trackers,trackers_pre;
int last_trackNo = 0;
//**计算速度
cv::Mat H;	//单应变换矩阵
...
/*语音播报进程*/
void *reportState_in_thread(void *ptr)
{
	myparam *param = (myparam*)ptr;
	reportTrafficCondition(param);
	return 0;
}
    ...
    int frame_skip, char *prefix, char *out_filename, int mjpeg_port, int json_port, int dont_show, int ext_output, int letter_box_in)
{
	...
	/*创建进程句柄*/
	pthread_t reporttrafficState_thread;
	/*进程参数结构体定义*/
	myparam* param = (myparam*)calloc(1, sizeof(myparam));
	param->isAnotherthreadthere = false;
	...
	cv::Mat* getsize_p = (cv::Mat*) in_img;
    int count = 0;
    if(!prefix && !dont_show){
        int full_screen = 0;
        create_window_cv("Demo", full_screen, getsize_p->cols, getsize_p->rows);
    }
    ...
    while(1){
        ++count;
        {
			...
			//***make the detections' copy
			makecopy_detection(local_dets, dets_copy, demo_thresh,l.classes);
			//update trackers
			tracker_update(trackers, trackers_pre,dets_copy, frame_id, last_trackNo);
			//get speed
			float averageSpeed;
			averageSpeed = getSpeed(trackers, H, *(cv::Mat*)in_img);
			param->averageSpeed = averageSpeed;
			param->frame = frame_id;

			if(!param->isAnotherthreadthere && frame_id>80)
				if (pthread_create(&reporttrafficState_thread, 0, reportState_in_thread, param)) error("Thread creation failed");
            ...
            draw_detections_cv_v3_my(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output,fps,trackers,averageSpeed);
 ...
 /*           if(delay == 0){
			...
			//double timecost = after - before;
			//int nmstowait = int((41666. - timecost) / 1000);
			//nmstowait = (nmstowait < 0) ? 1 : nmstowait;
			//Sleep(nmstowait);
			do
			{
				after = get_time_point();
			} while ((after - before) < 41000.);//保证24帧速率
    ...
    free_network(net);
    //cudaProfilerStop();
	myread("再见!交通事件检测任务退出！");
}

/*
**@function:make a copy of the detection
*/
