...
// Draw Detection
// ====================================================================
void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, float fps)
{
	//static int wait;		//waitKey(wait),让视频流以正常速率播放
    try {
        cv::Mat *show_img = mat;
        int i, j;
        if (!show_img) return;
		...
		//左上角显示帧数
		cv::Point pt_text_fps;
		pt_text_fps.x = 15;
		pt_text_fps.y = 15 + (show_img->rows / 1000.F);
		char str_fps[10];
		sprintf_s(str_fps, 10, "FPS:%0.2f", fps);
		cv::putText(*show_img, str_fps, pt_text_fps, cv::FONT_HERSHEY_COMPLEX_SMALL, (show_img->rows / 1000.F), CV_RGB(0, 255, 0), 2 * (show_img->rows / 1000.F), CV_AA);
        if (ext_output) {
            fflush(stdout);
        }
		/*wait = 40 - int(1000.0 / fps + 0.5);
		cv::waitKey(wait);*/
    }
    catch (...) {
...
//***
void draw_detections_cv_v3_my(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, float fps, std::vector<tracker> trackers, float averageSpeed)
{
	//static int wait;		//waitKey(wait),让视频流以正常速率播放
	try {
		cv::Mat *show_img = mat;
		...
		char str_fps[10];
		sprintf_s(str_fps, 10, "FPS:%0.2f", fps);
		char str_averageSpeed[20];
		sprintf_s(str_averageSpeed, 20, "AverageSpeed:%.1f", averageSpeed);
		cv::putText(*show_img, str_fps, pt_text_fps, cv::FONT_HERSHEY_COMPLEX_SMALL, (show_img->rows / 1000.F), CV_RGB(0, 255, 0), 2 * (show_img->rows / 1000.F), CV_AA);
		pt_text_fps.y = pt_text_fps.y + 10;
		cv::putText(*show_img, str_averageSpeed, pt_text_fps, cv::FONT_HERSHEY_COMPLEX_SMALL, (show_img->rows / 1000.F), CV_RGB(0, 255, 0), 2 * (show_img->rows / 1000.F), CV_AA);
		if (ext_output) {
			fflush(stdout);
		}
		...
		for (auto p = trackers.begin(); p != trackers.end(); p++)
		{
			...
			char str_trackNo[15];
			char str_speed[10];
			for (auto p1 = p->trace.begin() + 1; p1 != p->trace.end(); p1++)
			{
				pt1.x = int((p1 - 1)->x * show_img->cols);
				pt1.y = int((p1 - 1)->y * show_img->rows);
				pt2.x = int(p1->x * show_img->cols);
				pt2.y = int(p1->y * show_img->rows);
				cv::line(*show_img, pt1, pt2, CV_RGB(255, 255, 0), 2);
			}
			sprintf(str_trackNo, "No.%-5d", p->tracker_No);
			sprintf(str_speed, "speed:%.1f", p->speed);
			cv::putText(*show_img, str_trackNo, pt2, cv::FONT_HERSHEY_COMPLEX_SMALL, (show_img->rows / 700.F), CV_RGB(0, 255, 0), 2 * (show_img->rows / 1000.F), CV_AA);
			pt2.y += 12;
			cv::putText(*show_img, str_speed, pt2, cv::FONT_HERSHEY_COMPLEX_SMALL, (show_img->rows / 700.F), CV_RGB(0, 255, 0), 2 * (show_img->rows / 1000.F), CV_AA);
		}
}
	catch (...) {
		cerr << "OpenCV exception: draw_detections_cv_v3() \n";
